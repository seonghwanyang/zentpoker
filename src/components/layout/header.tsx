...
export function Header() {
  // Supabase Auth 정보 가져오기
  const { user, session, loading, signOut, isAdmin, role } = useAuth();
  // 모바일 메뉴 열림/닫힘 상태
          ...
          <div className="flex items-center space-x-4">
            {loading ? (
              ...
              <div className="flex items-center space-x-3">
                <MemberBadge grade={role === 'ADMIN' ? 'ADMIN' : 'BRONZE'} />
            ...
            <nav className="flex flex-col space-y-4">
              {user ? (
                <>
                  ...
                  {(isAdmin || role === 'ADMIN' || user?.user_metadata?.memberGrade === 'ADMIN') && (
                    <Link
